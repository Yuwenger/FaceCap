...
class CameraPreview extends SurfaceView implements SurfaceHolder.Callback {
    ...
    void takePictureInternal(final File img_file) {
        safeToTakePicture = false;
        if (mCamera != null) {
            mCamera.takePicture(null, null, new Camera.PictureCallback() {
                @Override
                        ...
                        public void run() {
                            ...
                            if (HiddenCameraUtils.saveImageFromFile(rotatedBitmap,
                                ...
                                new android.os.Handler(Looper.getMainLooper()).post(new Runnable() {
                                    @Override
                                    public void run() {
                                        mCameraCallbacks.onImageCapture(img_file);
                                    }
                                });
                            } else {
                                //Post error to the main thread
    ...
    void takePictureInternal(final String img_name) {
        final File img_file = new File(img_name);
        safeToTakePicture = false;
        if (mCamera != null) {
            mCamera.takePicture(null, null, new Camera.PictureCallback() {
                @Override
